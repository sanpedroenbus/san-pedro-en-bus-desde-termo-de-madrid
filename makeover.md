...
| P25 | ~~**Confidence indicator removed.**~~ **DONE.** `getConfidence()`/`confidence.ts` deleted along with the `ConfidenceBadge` UI, the `common.confidence`/`low`/`medium`/`high` and `methodology.confidence*` dictionary keys, and all references in `PRODUCT.md`/`DESIGN.md`. Rationale: with reports still early and low-volume, the count-based confidence badge mechanically penalized the exact behavior the product wants (more reporting) — every new report either kept confidence "low" or barely nudged it, while implying a validity judgment the sample size can't support. Report counts and recency (`latestReportAt`) now carry that signal instead, plus an explicit "this is early data" bullet added to the methodology copy | Done |
| P26 | **New feature: problem filter on /explorar.** `FilterBar` gained a category-grouped problem multi-select (mirrors `ProblemSelector`'s report-form UI), URL param `problema` (comma-separated problem ids, parsed via new `parseSelectedProblems()` in `dashboard-query.ts`). Threaded through the full server chain the same way `routes` already was: `DashboardModuleSearch.problems`, `.overlaps("problems", …)` on the Supabase query, `reportMatchesProblems()` (new, `src/lib/domain/reports.ts`) on the memory-store path, and the cache key (`normalizeDashboardCacheKey` now returns `problemsKey` too — **every caller of `getCachedExplorePageData`/`getCachedUnitDetail`/`getCachedRouteDetail` took a new positional `problemsKey` arg**). Filtering is "any overlap," not "only this problem": a matched report keeps all of its other problems too, so e.g. filtering by "cucarachas" can still show a nonzero "hacinados" bar if a matched report also had hacinados. This surprised the first e2e assertion for it (written assuming problem-exclusive reports) — fixed by asserting the filtered total only ever narrows the unfiltered total, not by asserting specific bars disappear, since that depends on real (mutable) data co-occurrence, not just the filter logic. Domain logic unit-tested directly (`reportMatchesProblems` in `reports.test.ts`, `parseSelectedProblems` in `dashboard-query.test.ts`) | Done |
| P27 | **New feature: external open-ended report link.** `/reportar` now links out to an external form (Google Form, placeholder URL — real link still needed, see `OPEN_REPORT_FORM_URL` in `report-form.tsx`) for reports that don't fit the fixed 16-problem catalogue, instead of adding an open-text field to the reports table (which `AGENTS.md`/`PRODUCT.md` forbid without a deliberate product-doc revision). `PRODUCT.md` gained a new "Open-ended reports" subsection spelling out the constraint: not wired into the reports API/table, never counted in any dashboard aggregate, and if ever surfaced in-app (e.g. a raw list on /explorar) must stay visually/structurally separate from the counted statistics. **Not yet implemented**: the actual "list these separately on /explorar" half of the original request — only the outbound link exists so far | Link needed from the user; the /explorar listing half is not started |
| P28 | ~~**BUG (found and fixed this session) — `globals.css`'s `.home-train-silhouette` mask still pointed at `landing-bus.svg` after it was deleted.**~~ **FIXED.** The user replaced `landing-bus.svg` with a resized, background-keyed `landing-bus.png` (checkerboard background removed by luminance-based alpha keying, cropped, downscaled, palette-quantized — 640KB→142KB) and asked for the SVG's removal. The OG-image route (`api/og/route.tsx`) was the only reference caught by a straightforward grep at the time; `globals.css`'s CSS `mask: url("/landing-bus.svg")` on the home page's decorative silhouette was missed and only surfaced later as a 404 caught by `visual.spec.ts`'s console-error assertion. Fixed by pointing the mask at the new PNG (mask works fine against a raster image's alpha channel, not just SVG). **Process note:** grep for a deleted asset's filename should include `.css`, not just `.ts`/`.tsx`, next time | Done — verified via Playwright, no more 404 |
| P29 | **Local `.env` has live Supabase credentials configured** (`NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`) — local dev in this session was hitting the real database, not the in-memory seed store, contrary to an initial assumption made early in this session (based on a commit message alone, not on checking `.env`). This matches P22/P23's own account of wave-3 verification being run "against real Supabase," so it's the current deliberate state, not a regression — but it means **the live database's data no longer matches the fixed seed data**: `mvp.spec.ts`'s "two heaviest seeded routes" test now fails (`San Ramón`/`La Europa` are no longer top-2; confirmed pre-existing on a clean checkout, not introduced this session) because real/test submissions have accumulated since the seed was written. Worth a decision: either reseed/reset the live dev project periodically, point local dev back at the memory store for reliable test fixtures, or rewrite that e2e assertion to not depend on exact seeded ranking | Needs a decision — test is currently red for a real, understood reason |
