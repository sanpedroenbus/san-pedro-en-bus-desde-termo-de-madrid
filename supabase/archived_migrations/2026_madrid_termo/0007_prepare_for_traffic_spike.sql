-- Keep production dashboard aggregates aligned with the app-owned fleet
-- qualification rule and Termo Indicator saturation parameters.
...
as $$
  ...
  visible_car_counts as (
    ...
    from visible
    where visible.car is not null
    group by visible.line, visible.car
  ),
  visible_counts as (
    select
      visible.line,
      ...
      (
        ...
        from visible_car_counts
        where visible_car_counts.line = visible.line
          and visible_car_counts.calor_reports + visible_car_counts.infierno_reports - visible_car_counts.fresco_reports > 2
      ) as cars_without_ac_reported,
  ...
  diagnostics as (
    select
      ...
      case
        when coalesce(line_weights.effective_reports, 0.0) <= 0 then 0.0
        else 100.0 * (1.0 - power(2.0, -line_weights.effective_reports / 30.0))
      end as report_score,
  ...
  select
    ...
    round(((diagnostics.weighted_heat / 100.0) * (
      ...
      0.35 * (case when diagnostics.weighted_fleet_percentage <= 0 then 0.0 else 100.0 * (1.0 - power(2.0, -diagnostics.weighted_fleet_percentage / 30.0)) end)
    ...
    round((case when diagnostics.weighted_fleet_percentage <= 0 then 0.0 else 100.0 * (1.0 - power(2.0, -diagnostics.weighted_fleet_percentage / 30.0)) end)::numeric, 2)::double precision as fleet_score
  from selected_lines
