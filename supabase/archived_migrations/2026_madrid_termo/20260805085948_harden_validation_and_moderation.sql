alter table public.reports
  drop constraint if exists reports_car_check;

alter table public.reports
  add constraint reports_car_check
  check (car is null or car ~ '^[MRS][0-9]{4,5}$') not valid;

alter table public.cars
  drop constraint if exists cars_code_check;

alter table public.cars
  add constraint cars_code_check
  check (code ~ '^[MRS][0-9]{4,5}$') not valid;
